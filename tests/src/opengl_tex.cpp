...
int main() {
    // Create window
    ex::Window& window = ex::Window::create({ 800, 600 }, "OpenGL Tex Test");
    if (!window.is_open()) {
    ...
    ex::gl::Tex texture({ tex_w, tex_h }, checker.data());
    texture.set_filter(ex::gl::Tex::Filter::Nearest, ex::gl::Tex::Filter::Nearest);
    texture.set_wrap(ex::gl::Tex::Wrap::Repeat, ex::gl::Tex::Wrap::Repeat);
